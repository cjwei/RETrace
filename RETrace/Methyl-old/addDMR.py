...
def filterDMR(tsv_file, DMRdict, output_file, CGN_flag):
    tsv_output = open(output_file, 'w')
    with open(tsv_file) as f_tsv:
        for i, line in enumerate(tqdm(f_tsv)):
            (chr, pos, strand, C_type) = line.split()[0:4]
            if CGN_flag is True: #We want to skip line if not CGN methylation signal if CGN_flag is True
                if C_type not in ["CGN", "CGA", "CGT", "CGC", "CGG"]:
                    continue
            if chr.replace('chr','') not in DMRdict.keys():
                tsv_output.write(line) #We want to still print out the CpG information even if chromosome not found in DMR bed (particularly, this would otherwise cause error in cell line processing)
                continue
...
def main():
    ...
    parser.add_argument('-CGN', action="store_true", default=True, help="Flag specifying we only want to keep tsv_file lines that contain CGN methylation information")
    args = parser.parse_args()

    DMRdict = importDMR(args.DMR_file)
    filterDMR(args.tsv_file, DMRdict, args.output_file, args.CGN)

...
def calcTriplets(NJTree, exvivo_pd, sampleDict, metric, plot_file):
    analyzed_triplets = []
    errorDict = {}
            ...
            for sample3 in sorted(sampleDict.keys()):
                ...
                if len(set([clone1, clone2, clone3])) == 3 and triplet not in analyzed_triplets: #We want only triplets where each of the three leaves stem from different clones
                    ...
                    if metric == "Root":
                        ...
                        for sample_pair in ([0,1], [0,2], [1,2]):
                            pairwise_dist = NJTree.get_distance(triplet[sample_pair[0]], triplet[sample_pair[1]])
                            tree_dist.append(pairwise_dist)
                            ref_dist.append(exvivo_pd.loc[sampleDict[triplet[sample_pair[0]]]["clone"]][sampleDict[triplet[sample_pair[1]]]["clone"]])
                        if min(ref_dist) not in errorDict.keys():
                            errorDict[min(ref_dist)] = {}
                            errorDict[min(ref_dist)]["Correct"] = 0
                            errorDict[min(ref_dist)]["Total"] = 0
                        if tree_dist.index(max(tree_dist)) == ref_dist.index(min(ref_dist)): #If using pairwise distance as comparison metric
                            errorDict[min(ref_dist)]["Correct"] += 1 #If using pairwise distance as comparison metric
                        errorDict[min(ref_dist)]["Total"] += 1 #If using pairwise distance as comparison metric
                    else:
                        print("Unknown distance metric:\t" + metric)
                        return
...
def main():
    ...
    parser.add_argument('--exVivo', action="store", dest="exVivo_dist", help="csv file containing pairwise cell division distances across ex vivo tree.  Choose based on metric (MRCA = *.MRCADist.csv; Pairwise Dist = *pairwiseDist.csv)")
    parser.add_argument('--tree', action="store", dest="tree_file", help="Newick tree file")
    parser.add_argument('--metric', action="store", dest="metric", help="[Root/Pairwise] Distance metric either with cell division difference (Pairwise) vs MRCA depth (MRCA)")
    ...
    errorDict = calcTriplets(NJTree, exVivo_pd, sampleDict, args.metric, args.plot_file)
